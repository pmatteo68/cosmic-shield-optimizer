...
opt_log=${OPT_LOGS_DIR}/css-opt-dummy-${curr_tstamp}.log

export CSS_OPT_STOPFILE=./css-opt-dummy-stop-${curr_tstamp}.txt

nohup ./css-optimizer-dummy.sh > ${opt_log} 2>&1 &
#nohup ./css-optimizer.sh > ${opt_log} 2>&1 &

ppid=$!
echo "[$(date)] CSS Optimizer (dummy) process launched in background (pid: ${ppid})"
echo
